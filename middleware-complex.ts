/**
 * Security Middleware with NextAuth Integration
...
const MIDDLEWARE_CONFIG = {
  ...
  publicRoutes: ['/api/health', '/api/status', '/api/auth/reset-rate-limit', '/', '/docs'],
  authRoutes: ['/api/auth', '/auth'],
  staticRoutes: ['/_next', '/favicon.ico', '/robots.txt', '/sitemap.xml'],
  
  // Routes that require authentication
  protectedRoutes: ['/dashboard', '/orders', '/menu', '/inventory', '/staff', '/analytics', '/reservations', '/pos', '/settings', '/admin', '/reports'],
...
export async function middleware(request: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    // Apply CSRF protection for state-changing requests (skip for reset endpoint in dev)
    const skipCSRF = process.env.NODE_ENV === 'development' && pathname === '/api/auth/reset-rate-limit';
